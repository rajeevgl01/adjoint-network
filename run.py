...
def updateSequenceOutput(output, ngpu=4):
    ...
    if ngpu == 1:
        ...
        first = torch.cat([output[:l//factor], output[2*l//factor:3*l //
                          factor], output[4*l//factor:5*l//factor]], dim=0)
        second = torch.cat([output[l//factor:2*l//factor], output[3*l //
                           factor:4*l//factor], output[5*l//factor:]], dim=0)
    elif ngpu == 4:
        first = torch.cat([output[:l//factor], output[2*l//factor:3*l//factor],
                          output[4*l//factor:5*l//factor], output[6*l//factor:7*l//factor]], dim=0)
        second = torch.cat([output[l//factor:2*l//factor], output[3*l//factor:4*l//factor],
                           output[5*l//factor:6*l//factor], output[7*l//factor:]], dim=0)
...
class Runner():
    ...
    def one_batch(self, xb, yb, *args):
        try:
            ...
            if self.learn.teacher_model == None:
                ...
                self.loss = self.learn.loss_func(
                    self.teacher_pred, self.pred, self.yb)
            ...
            self.handle("after_cancel_batch")
        finally:
    ...
    def all_batches(self, dataloader):
        ...
        try:
            ...
            self.handle("after_cancel_epoch")
    ...
    def fit(self, epochs, train=True, start_epoch=0):
        ...
        try:
            ...
            for epoch in range(start_epoch, epochs):
                ...
                self.all_batches(self.learn.data.train)

                self.learn.model.eval()
                self.in_train = False
                with torch.no_grad():
                    self.all_batches(self.learn.data.valid)

                self.handle("after_epoch")

        except CancelTrainException:
            self.handle("after_cancel_train")
        finally:
