...
if __name__ == "__main__":
    ...
    if args.default_config == 'True':
        batch_size, image_size, lr, c, epoch, is_sgd = get_default_config(
            args.dataset, 4)
    else:
    ...
    if is_sgd:
        ...
        lr_sched = combine_schedules(
            [0.1, 0.9], [sched_cos(lr/10., lr), sched_cos(lr, lr/1e5)])
        beta1_sched = combine_schedules(
            [0.1, 0.9], [sched_cos(0.95, 0.85), sched_cos(0.85, 0.95)])
        lr_scheduler = ParamScheduler('lr', lr_sched)
        beta1_scheduler = ParamScheduler('beta1', beta1_sched)
        cbfs = [NormalizeCallback(device), lr_scheduler,
                beta1_scheduler, CudaCallback(device)]

    cbfs += [SaveModelCallback("ind-resnet100-256-4e-3",
                               save_dir="model_callback/")]
    if is_individual_training:
        ...
        if resnet == 18:
            ...
            model = xresnet100(c_out=c, resize=data_resize,
                               compression_factor=compression_factor)
        elif resnet == 101:
        ...
        resnet = args.resnet
        if resnet == 18:
            model = xresnet_fast18(
                c_out=c, resize=data_resize, compression_factor=compression_factor, masking_factor=masking_factor)
        elif resnet == 34:
            model = xresnet_fast34(
                c_out=c, resize=data_resize, compression_factor=compression_factor, masking_factor=masking_factor)
        elif resnet == 50:
            model = xresnet_fast50(
                c_out=c, resize=data_resize, compression_factor=compression_factor, masking_factor=masking_factor)
        elif resnet == 100:
            model = xresnet_fast50X2(
                c_out=c, resize=data_resize, compression_factor=compression_factor, masking_factor=masking_factor)
        elif resnet == 101:
            model = xresnet_fast101(
                c_out=c, resize=data_resize, compression_factor=compression_factor, masking_factor=masking_factor)
        elif resnet == 152:
            model = xresnet_fast152(
                c_out=c, resize=data_resize, compression_factor=compression_factor, masking_factor=masking_factor)
        else:
            print("Resnet model supported are 18, 34, 50, 101, 152")
    ...
    if last_epoch_done_idx is not None:
        model = load_model(
            model, state_dict_file_path="/scratch/un270/model/Adjoint-Experiments/Feb2020/Ind-resnet50-imagenet-256-1e-3/{}.pt".format(last_epoch_done_idx))
    ...
    if is_student_teacher:
        ...
        teacher_model = load_model(
            teacher_model, state_dict_file_path='/scratch/un270/model/Adjoint-Experiments/{}-individual-50-bs32/{}.pt'.format(args.dataset, 144))
        teacher_model.cuda()
    ...
    if is_sgd:
        ...
        opt = StatefulOptimizer(model.parameters(), [weight_decay, adam_step], stats=[AverageGrad(
        ), AverageSqGrad(), StepCount()], lr=0.001, wd=1e-2, beta1=0.9, beta2=0.99, eps=1e-6)
    ...
    with torch.cuda.amp.autocast(enabled=True):
        run.fit(
            epoch, start_epoch=0 if last_epoch_done_idx is None else last_epoch_done_idx+1)
