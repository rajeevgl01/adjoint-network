...
def conv_layer(ni, no, ks, s, zero_bn=False, act=True, expansion=1, compression_factor=4, masking_factor=None):
    ...
    layers = [conv(ni, no, ks=ks, s=s, expansion=expansion,
                   compression_factor=compression_factor, masking_factor=masking_factor), bn]
    if act:
        layers += [act_func]
...
class ResBlock(nn.Module):
    def __init__(self, ni, no, expansion, compression_factor, masking_factor, s=1):
        super().__init__()
        print('Res ni {}, no {}'.format(ni, no))
        ni *= expansion
        layers = [conv_layer(ni, no, 3, s, expansion=expansion, compression_factor=compression_factor, masking_factor=masking_factor),
                  conv_layer(no, no*expansion, 3, 1, zero_bn=True, act=False, expansion=expansion,
                             compression_factor=compression_factor, masking_factor=masking_factor)
                  ] if expansion == 1 else [
            conv_layer(ni, no, 1, 1, expansion=expansion,
                       compression_factor=compression_factor, masking_factor=masking_factor),
            conv_layer(no, no, 3, s, expansion=expansion,
                       compression_factor=compression_factor, masking_factor=masking_factor),
            conv_layer(no, no*expansion, 1, 1, zero_bn=True, act=False, expansion=expansion,
                       compression_factor=compression_factor, masking_factor=masking_factor)
        ]
        self.convs = nn.Sequential(*layers)
        self.idconv = noop if ni == no*expansion else conv_layer(
            ni, no*expansion, 1, 1, act=False, expansion=expansion, compression_factor=compression_factor, masking_factor=masking_factor)
...
class XResNet(nn.Sequential):
    ...
    def create(cls, expansion,  layers, c_in=3, c_out=10, resize=cifar_resize, compression_factor=4, masking_factor=None):
        ...
        layers.extend([nn.AdaptiveAvgPool2d(1), Flatten(),
                      linear(nbs[-1]*expansion, c_out)])
...
def xresnet_fast50X2(**kwargs): return XResNet.create(4,
                                                      [6, 8,  12, 6], **kwargs)
def xresnet_fast101(**kwargs): return XResNet.create(4,
                                                     [3, 4, 23, 3], **kwargs)
