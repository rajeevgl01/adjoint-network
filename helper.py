...
def load_cifar_data(batch_size, image_size, size):
    ...
    tfms = aug_transforms(do_flip=True, flip_vert=False, max_rotate=25)
    data = ImageDataLoaders.from_folder(
        path, valid='test', batch_tfms=[Normalize.from_stats(*imagenet_stats), *tfms], bs=batch_size)
...
def load_data(batch_size, image_size, dataset=1):
    ...
    train_transforms = [make_rgb, RandomResizedCrop(
        image_size, scale=(0.35, 1)), PilRandomFlip(), np_to_float]
    valid_transforms = [make_rgb, CenterCrop(image_size), np_to_float]

    data = Data(path, batch_size=batch_size, image_transforms=train_transforms,
                valid_image_transforms=valid_transforms, num_workers=20)
    print("Loaded data")
    return data
...
def load_fastai_data(batch_size, image_size):
    ...
    pat = r'/([^/]+)_\d+.jpg$'
    data = ImageDataLoaders.from_name_re(path_img, fnames, pat, ds_tfms=get_transforms(
    ), size=image_size, bs=batch_size).normalize(imagenet_stats)
