...
class conv2dFirstLayer(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, padding, stride, *kargs, **kwargs):
        super(conv2dFirstLayer, self).__init__(in_channels, out_channels,
                                               kernel_size, padding, stride, *kargs, **kwargs)
        self.padding = (padding, padding)
        self.stride = (stride, stride)

    def forward(self, input):
        a = F.conv2d(input, self.weight, self.bias, self.stride,
...
class conv2dAdjoint(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, padding, stride, mask_layer, compression_factor=1, masking_factor=None, *kargs, **kwargs):
        super(conv2dAdjoint, self).__init__(in_channels, out_channels,
                                            kernel_size, padding, stride, *kargs, **kwargs)
        self.padding = (padding, padding)
        self.stride = (stride, stride)
    ...
    def forward(self, input):
        l, _, _, _ = input.shape
        # print("in an.py file forward function con2dadjoint")
        # print(input.shape)
        a = F.conv2d(input[:l//2], self.weight,
                     self.bias, self.stride, self.padding)
        if self.mask_layer:
            b = F.conv2d(input[l//2:], self.weight*self.mask,
                         self.bias, self.stride, self.padding)
...
class batchNorm(nn.Module):
    ...
    def forward(self, input):
        l, _, _, _ = input.shape
        # print("in an.py file forward function batchNorm")
        # print(input.shape)
...
class linear(nn.Linear):
    def __init__(self, in_features, out_features, parts=4, bias=True, *kargs, **kwargs):
        super(linear, self).__init__(in_features,
                                     out_features, bias=True, *kargs, **kwargs)
