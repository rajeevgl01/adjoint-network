...
class CudaCallback(CallBacks):
    ...
    def begin_batch(self): self.run.xb, self.run.yb = self.xb.to(
        self.device), self.yb.to(self.device)
...
class Stats():
    ...
    def avg_stats(self): return [
        s/self.count if self.count != 0 else -1.0 for s in self.all_stats]
...
class ParamScheduler(CallBacks):
    ...
    def set_param(self):
        ...
        for h in hypers:
            h[self.pname] = self.sched_func(
                (self.iter+self.start_epoch)/self.epochs)
...
class LR_find(CallBacks):
    ...
    def after_cancel_train(self):
        print("Best loss value = {} at iteration = {}".format(
...
class NormalizeCallback(CallBacks):
    ...
    def begin_batch(self):
        self.run.xb = (
            self.xb - self.mean[..., None, None]) / self.std[..., None, None]
...
class DebugTimeCallback(CallBacks):
    ...
    def print_times(self):
        times = [self.data_loading_time, self.model_compute_time,
                 self.loss_compute_time, self.backward_compute_time, self.opt_step_time]
